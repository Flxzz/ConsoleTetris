...
{
	if (player == PLAYER1)
		CurrentBlock1.DrawBlock(m_board, m_width, m_board_x);
	else
		CurrentBlock2.DrawBlock(m_board, m_width, m_board_x);
}

inline void CUI::ClearCurrentBlock(Player player, bool print)
{
	if (player == PLAYER1)
		print ? CurrentBlock1.ClearBlock(m_board, m_width, m_board_x) : CurrentBlock1.ClearBlockNotPrint(m_board, m_width);
	else
		print ? CurrentBlock2.ClearBlock(m_board, m_width, m_board_x) : CurrentBlock2.ClearBlockNotPrint(m_board, m_width);
}
...
void CUI::GetAProp()
{
	...
	{
	case 0:
		if (m_strip_number < 1000)
			m_strip_number++;
		break;
	case 1:
		if (m_bomb_number < 1000)
			m_bomb_number++;
		break;
	case 2:
		if (m_pierce_number < 1000)
			m_pierce_number++;
		break;
	}
}
...
inline bool CUI::GameOver(Player player) const
{
	if (player == PLAYER1)
	{
		if (CurrentBlock1.GetBlockType() == B_BOMB)
			return false; // 当前方块是炸弹时不会使游戏结束
		else
			return CurrentBlock1.BlockCollisionTest(m_board, m_width); // 如果当前方块在一出来的时候就检测到碰撞，则游戏结束
	}
	else
	{
		if (CurrentBlock2.GetBlockType() == B_BOMB)
			return false; // 当前方块是炸弹时不会使游戏结束
		else
			return CurrentBlock2.BlockCollisionTest(m_board, m_width); // 如果当前方块在一出来的时候就检测到碰撞，则游戏结束
	}
}
...
void CUI::ShowInfo() const
{
	...
	if (m_strip_number > 0)
	{
		...
		PrintString(L"           ", x_left_info, y_left_info + 6, WHITE);
	if (m_bomb_number > 0)
	{
		...
		PrintString(L"           ", x_left_info, y_left_info + 7, WHITE);
	if (m_pierce_number > 0)
	{
		...
		PrintString(L"             ", x_left_info, y_left_info + 8, WHITE);
...
void CUI::LoadSettings()
{
	...
	if (m_clear_animation_speed < 5)
		m_clear_animation_speed = 40; // 如果载入了非法的值，则设置成默认值
	m_allow_level_up = (GetIntFromIni("配置", "允许升级", 1) != 0);
	m_score_per_level = GetIntFromIni("配置", "提升1级所需得分", 150);
	if (m_score_per_level < 1)
		m_score_per_level = 150; // 如果载入了非法的值，则设置成默认值
	...
	if (m_invisible_mode_difficulty > 2)
		m_invisible_mode_difficulty = 2;
	if (m_invisible_mode_difficulty < 0)
		m_invisible_mode_difficulty = 0;
...
void CUI::OptionSettings()
{
	...
	while (true)
	{
		...
		{
		case 0:
			PrintString(L"低", x + 9, y + 8, YELLOW);
			break;
		case 1:
			PrintString(L"中", x + 9, y + 8, YELLOW);
			break;
		case 2:
			PrintString(L"高", x + 9, y + 8, YELLOW);
			break;
		default:
			break;
		}
		...
		case UP_KEY:
			item_select--;
			if (item_select < 1)
				item_select = max_selection;
			break;
		case DOWN_KEY:
			item_select++;
			if (item_select > max_selection)
				item_select = 1;
			break;
		case ENTER_KEY:
		case LEFT_KEY:
		case RIGHT_KEY:
			...
			{
			case 1:
				m_clear_animation = !m_clear_animation;
				break;
			case 2:
				GotoXY(x + 17, y + 3);
				CursorVisible(true);
				...
				if (m_clear_animation_speed < 5)
					m_clear_animation_speed = 5; // 消除动画的间隔时间不允许小于5毫秒
				if (m_clear_animation_speed > 9999)
					m_clear_animation_speed = 9999; // 消除动画的间隔时间不允许大于9999毫秒
				CursorVisible(false);
			...
			case 4:
				...
				if (m_score_per_level < 1)
					m_score_per_level = 1; // 每升一级需要的得分不允许小于1
				CalculateLevel();
				CursorVisible(false);
...
void CUI::ColorSettings()
{
	...
	while (true)
	{
		...
		case UP_KEY:
			color_select--;
			if (color_select < 1)
				color_select = max_selection;
			break;
		case DOWN_KEY:
			color_select++;
			if (color_select > max_selection)
				color_select = 1;
			break;
		case ENTER_KEY:
		case RIGHT_KEY:
